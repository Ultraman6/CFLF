...
class Sub_Exp_Num_API(BaseServer):
    ...
    def train(self, task_name, position):
        ...
        for round_idx in tqdm(range(1, self.args.round+1), desc=task_name, leave=False):
            ...
            self.global_params = self.quality_detection(w_locals, round_idx, num_weights)
            # 更新权重并聚合
    ...
    def quality_detection(self, w_locals, round_idx, num_weights):  # 基于随机数结合概率判断是否成功返回模型
