...
class Cross_Up_Num_API(BaseServer):
    ...
    def train(self, task_name, position):
        ...
        for round_idx in tqdm(range(1, self.args.round+1), desc=task_name, leave=False):
            ...
            # 质量检测
            num_weights = np.array([self.sample_num[cid] / self.all_sample_num for cid in client_indexes])
            self.global_params = self.quality_detection(w_locals, round_idx, test_loss, test_acc, num_weights)
    ...
    def quality_detection(self, w_locals, round_idx, test_loss, test_acc, num_weights):  # 基于本地更新前模型的测试损失
        # 质量检测:先计算全局损失，再计算每个本地的损失
        weights, loss_update = [], []  # 用于存放边际损失
        with ThreadPoolExecutor(max_workers=len(w_locals)) as executor:
            ...
            for cid, w in enumerate(w_locals):
                ...
                futures.append(future)
            for future in futures:
                loss_up, weight = future.result()
                loss_update.append(loss_up)
                weights.append(weight * num_weights[cid])
    ...
    def compute_margin_values(self, w_i, valid_global, model_trainer, test_loss, test_acc):
        ...
        margin_loss = test_loss - loss_i
        margin_metric = margin_loss
        return margin_loss, np.exp(self.gamma * margin_metric)
