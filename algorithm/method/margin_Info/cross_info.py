...
class Margin_Cross_Info_API(BaseServer):
    ...
    def quality_detection(self, w_locals, round_idx):  # 基于随机数结合概率判断是否成功返回模型
        ...
        acc, loss = self.model_trainer.test_info(self.valid_global)
        ...
        for i, w in enumerate(weights):
            ...
            self.quality_info[i][round_idx] = {
                "margin_cross_info": margin_loss[i],
                "quality": weights[i],
                "weight": w / total_w
            }
    ...
    def compute_margin_values(self, w_locals_i, valid_global, model_trainer, loss):
        ...
        acc_i, loss_i = model_trainer.test_info(valid_global)
