...
class Grad_Norm_Update_API(BaseServer):
    ...
    def quality_detection(self, w_locals, round_idx):  # 基于随机数结合概率判断是否成功返回模型
        ...
        with ThreadPoolExecutor(max_workers=len(self.client_list)) as executor:
            ...
            for cid, future in enumerate(futures):
                kl_gn_up, weight = future.result()
                margin_kl_gn.append(kl_gn_up)
        ...
        for i, w in enumerate(weights):
            ...
            self.quality_info[i][round_idx] = {
                "KL_Grad_Norm_Update": margin_kl_gn[i],
                "Exp": weights[i],
                "weight": w / total_w
            }
    ...
    def compute_margin_values(self, w_i, valid_global, model_trainer, kl_gn):
        model_trainer.set_model_params(w_i)
        acc_i, loss_i, kl_gn_i = model_trainer.test_norm(valid_global)
        kl_gn_up = kl_gn_i - kl_gn
        return kl_gn_up, np.exp(self.gamma * kl_gn_up)  # 试试看
