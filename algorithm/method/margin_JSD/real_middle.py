...
class Margin_JSD_Real_Middle_API(BaseServer):
    ...
    def quality_detection(self, w_locals, round_idx):  # 基于随机数结合概率判断是否成功返回模型
        ...
        acc, loss, jsd_g = self.model_trainer.test_direct_KL(self.valid_global)
        ...
        for i, q in enumerate(quality):
            ...
            self.quality_info[i][round_idx] = {
                "Margin_direct_kl_sum": margin_JSD[i],
                "quality": q,
                "weight": w
            }
    ...
    def compute_margin_values(self, w_locals_i, valid_global, model_trainer, jsd_g):
        ...
        acc_i, loss_i, jsd_i = model_trainer.test_direct_KL(valid_global)
