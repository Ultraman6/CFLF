...
class DITFE_API(BaseServer):
    ...
    def client_select(self):
        ...
        for cid, pay in pays:
            self.task.control.set_info('global', 'pay_info', (cid, {'支付': pay, '效用': pay - self.his_cost[cid],
            ...
            self.task.control.set_info('global', 'util_info',
                                       (self.his_bids[cid], {'支付': pay, '效用': pay - self.his_cost[cid],
                                                             '真实成本': self.his_cost[cid]}))
    ...
    def global_initialize(self):
        ...
        for cid in self.client_indexes:
            ...
            self.ucb_rewards[cid] = self.emp_rewards[cid] + np.sqrt(
                (self.k + 1) * np.log(1) / self.client_selected_times[cid])
    ...
    def update_ucb(self):
        for cid in range(self.args.num_clients):
            ...
            if cid in self.client_indexes:
                self.emp_rewards[cid] = (self.emp_rewards[cid] * (beta - 1) + self.his_rewards[cid][
                    self.round_idx]) / beta
