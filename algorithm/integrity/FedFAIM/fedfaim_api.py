...
from algorithm.aggregrate import average_weights, average_weights_self
...
class FedFAIM_API(object):
    ...
    def upgrate_gradient(self, w_global, w_locals, w_global_new):
        for client in self.client_list:
            self.gradient_local[client.id] = getGradient(w_locals[client.id], w_global)
    ...
    def quality_detection(self, w_locals):  # 基于随机数结合概率判断是否成功返回模型
        ...
        for client in self.client_list:
            acc_i, loss_i = self._local_test_on_validation_set(average_weights(np.delete(w_locals, client.id)))
            margin_loss_i = loss_i - loss
            if margin_loss_i > self.threshold:
                client.n_pass += 1  # 获胜记录
                margin_loss.append(margin_loss_i)
                w_locals_pass.append(w_locals[client.id])
                pass_idx.append(client.id)
            else:
                client.n_fail += 1  # 失败记录 （之前没有记录失败，导致梯度分配失调造成后期的梯度消失）
    ...
    def Reward_Allocation(self):
        ...
        for client in self.client_list:
            ...
            r.append(self.contrib[client.id] / c_max * self.a * exp(self.b * exp(self.c * x)))
        r_max = max(r)
        ...
        for client in self.client_list:
            num = r[client.id] / r_max * g_global_flat.numel()
            g_local_flat = gradient_flatten(self.gradient_local[client.id])
