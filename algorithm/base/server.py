...
class BaseServer:
    ...
    def global_initialize(self):
        ...
        self.task.control.set_statue('text', "Round : 0 Test Loss : {:.4f} Test Accuracy : {:.4f}".format(test_loss, test_acc))
    ...
    def thread_train(self, cid):  # 这里表示传给每个客户的全局信息，不止全局模型参数，子类可以自定义，同步到client的接收
        self.task.control.set_statue('text', "Client {} training begin".format(cid))
        w = self.client_list[cid].local_train(self.round_idx, self.local_params[cid])
        self.task.control.set_statue('text', "Client {} training end".format(cid))
        return w
