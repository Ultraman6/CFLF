...
class ChatApp(Embedding):
    ...
    def __init__(self, openai_api_key, chat_history, embedding_files, index_files):
        super().__init__(openai_api_key, embedding_files, index_files)  # Call the initializer of the parent class
        ...
        self.llm = ConversationChain(llm=ChatOpenAI(model_name=app.storage.user.get('last_model', self.openai_models[0]),
                                                    openai_api_key=self.openai_api_key, temperature="0.1"), memory=self.memory) if openai_api_key != '' else None
    ...
    def chat_history_grid(self):
        ...
        with ui.column().classes("h-1/2 overflow-y-auto bg-white cursor-pointer").bind_visibility_from(self,"embedding_switch", value=False):
