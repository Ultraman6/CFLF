...
class run_ui:
    def __init__(self, experiment):
        ...
        for tid in self.experiment.task_info_refs:
            ...
            # ui.echart()
        ui.button('开始运行', on_click=self.experiment.run_experiment)
    ...
    def draw_global_infos(self):
        rounds = 0
        for id in self.infos_ref:
            this_rounds = len(self.infos_ref[id]['global_info']['Loss'])
            rounds = rounds if rounds > this_rounds else this_rounds
        options = {
            "xAxis": {
                "type": "category",
                "name": '轮次',
