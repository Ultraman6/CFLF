...
class Task:
    ...
    def run(self):
        ...
        self.algo = self.algo_class(self)  # 每次都是重新创建一个算法对象

        run_res = self.iterate()  # 得到运行结果
    ...
    def iterate(self):
        ...
        self.control.set_statue('progress', (self.args.round, self.algo.round_idx))
        while not self.pre_quit:
            next = self.watch_control()  # 先做任务状态检查
            if next == 0:  # 结束当前迭代
                pbar.reset()
                continue
            elif next == 1:  # 继续迭代
            ...
            if self.algo.round_idx <= self.args.round:
                if self.algo.round_idx == 0:
                    self.algo.global_initialize()
                else:
                    self.algo.iter()
                    pbar.update(1)  # 更新进度条
                    self.control.set_statue('progress', (self.args.round, self.algo.round_idx))
                    time.sleep(2)
                self.algo.round_idx += 1  # 更新 round_idx
            else:
                # 如果已经达到目标轮次，继续等待可能的 pre_quit
                continue

        self.algo.global_final()  # 善终操作
        pbar.close()  # 完成后关闭进度条
        if self.pre_quit:
